...
## Overview
This document describes all tables, columns, constraints, and relationships in the Bookstore database.
...
| address         | VARCHAR(50)    | -                                    | Customer mailing address |
| age             | INT            | CHECK (age >= 18 AND age <= 100)     | Customer age |
| income_level    | INT            | -                                    | Income level category |
...
| discount        | FLOAT          | CHECK (discount > 1.00 AND discount < 10.00) | Discount rate offered to bookstore |

### 3. Books
Stores book catalog and inventory details.

| Column           | Data Type      | Constraints                          | Description |
|------------------|----------------|--------------------------------------|-----------|
| isbn             | INT            | PRIMARY KEY                          | Unique book identifier (ISBN) |
| title            | VARCHAR(30)    | -                                    | Book title |
| author           | VARCHAR(20)    | -                                    | Book author name |
| qty_in_stock     | INT            | -                                    | Current quantity in stock |
| price            | FLOAT          | -                                    | Selling price |
| cost             | FLOAT          | -                                    | Cost price from publisher |
...
### 5. OrderList
Junction table linking orders to books (many-to-many relationship).
...
### 6. StockManager
Tracks current stock levels for books.
...
- One **Book** has **One** stock record in `StockManager` (1:1)
